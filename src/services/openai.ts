...
function parseResponse(raw: string): AnalysisResult {
  // Eliminar posibles bloques markdown que el modelo incluya a veces
  const cleaned = raw
    .replace(/^```json\s*/i, '')
    .replace(/^```\s*/i, '')
    .replace(/```\s*$/i, '')
  ...
  // Validación mínima de la estructura
...
export async function analyzeCV(req: AnalyzeRequest): Promise<AnalysisResult> {
  ...
  if (!raw) throw new Error('OpenAI no devolvió contenido en la respuesta');
